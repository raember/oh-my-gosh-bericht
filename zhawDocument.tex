...
The client expects to send all its input directly to the \gls{shell} without prior interpretation. To achieve this, the client-side terminal has to be set from the default \texttt{cooked}- into the \texttt{raw}-mode\citep[p.1309]{KerriskTLPI}. This sends each key stroke to the server-side shell as is. After the termination of the session, the original \texttt{cooked}-mode has to be restored to ensure operation as per usual.

\section{Pseudoterminal}\label{sec:Pseudoterminal}
Pseudoterminals help to solve the problem of how two remote programs can communicate as if they were directly connected via a terminal.
\begin{figure}[ht]
\includegraphics[width=\textwidth]{PseudoterminalProblem}
\caption{Diagram of two programs communicating via a pseudoterminal\citep[p.1376]{KerriskTLPI}}
\label{fig:SeqDiaOriginal}
\end{figure}
A shell expects to be connected to a full fledged terminal(or a terminal emulator). To check whether it is inside such an environment, it uses \cite{isatty} on the \gls{fd} it is connected to.
\begin{figure}[ht]
\includegraphics[width=\textwidth]{Pseudoterminal}
\caption{How pseudoterminals work\citep[p.1377]{KerriskTLPI}}
\label{fig:SeqDiaOriginal}
\end{figure}
