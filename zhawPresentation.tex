...
\section{The Problem}
\begin{zhawframe}
\onslide<1-> Design and implement an alternative to SSH (prototype)

\onslide<2-> Implementation language: Go (Golang)

\onslide<3-> Target platform: GNU/Linux
\end{zhawframe}

\section{The Present Solution}
\subsection{Telnet}
\begin{zhawframe}
\onslide<1-> telnet(1) is old (RFC15 1969, RFC854 1983)

\onslide<2-> No secure connection (except: TELNETS)
...
\onslide<2-> Secure connection (own protocol)
...
\begin{zhawframe}
\onslide<1-> Prevent MITM, provide integrity \& privacy
...
\onslide<4-> Failure in test environment $\rightarrow$ \texttt{login}(1)
...
\begin{zhawframe}
\onslide<1-> Authenticate via public key cryptography

\onslide<2-> Store authorized public keys on server

\onslide<3-> Authorized keys stored in \texttt{/root/.gosh} (plain-text)

\onslide<4->$\quad\rightarrow$ Hash in \texttt{\textasciitilde{}/.gosh/authorized\_{}keys}
...
\begin{zhawframe}
\onslide<1-> Shell should run with appropriate permissions (\texttt{setuid}(2)/\texttt{setgid}(2))

\onslide<2-> Failure to drop privileges after login (operation not supported)
...
\onslide<2-> \texttt{utmpx} $\rightarrow$ \texttt{w}/\texttt{who}

\onslide<3-> PAM: \texttt{pam\_{}open\_{}session}(3)/\texttt{pam\_{}close\_{}session}(3)
\end{zhawframe}
...
\onslide<3-> \texttt{/etc/passwd} (!) $\Rightarrow$ CGO: \texttt{getpwnam}(2)/\texttt{getpwuid}(2)
\end{zhawframe}
...
\begin{zhawframe}
\onslide<1-> \texttt{istty}(3) on the connected \texttt{fd}s
...
\onslide<3-> Wrapper function in internal(!) package of the Go standard library \texttt{os/signal/internal/pty}
\end{zhawframe}
...
\onslide<2-> Transfer of env vars (client $\leftrightarrow$ server)
...
\begin{zhawframe}
\onslide<1-> Forward all keystrokes without interpretation (client-sside)

\onslide<2-> \texttt{cooked} mode $\rightarrow$ \texttt{raw} mode
